#!/usr/bin/env python
"""
Clean Duplicate Category Images
Removes duplicate images created by Django's auto-generated suffixes
Keeps the original filename and removes duplicates like _abc123.png
"""
import os
import django
import argparse
...
def clean_duplicate_images(dry_run=True):
    """
    Clean duplicate category images, keeping only the original file
    """
    
    print(f"🧹 Clean Duplicate Category Images")
    print(f"Mode: {'🔍 DRY RUN (preview only)' if dry_run else '🚀 LIVE (making changes)'}")
    print("="*70)
    
    # Path to media categories
    media_categories_path = Path(settings.MEDIA_ROOT) / "categories"
    
    print(f"📁 Media Path: {media_categories_path}")
    
    if not media_categories_path.exists():
        print(f"❌ Error: Categories folder not found at {media_categories_path}")
        return None
    ...
    for file_path in media_categories_path.iterdir():
    ...
    report_filename = f'clean_duplicates{mode_suffix}_{timestamp}.txt'
    
    print(f"\n📝 Processing duplicates...")
    
    with open(report_filename, 'w', encoding='utf-8') as report:
        report.write(f"Clean Duplicate Category Images Report\n")
    ...
    if dry_run:
        ...
        print(f"   Your pages should load much faster now!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Clean Duplicate Category Images',
        epilog="""
Examples:
  python clean_duplicate_images.py                    # Dry run (preview)
  python clean_duplicate_images.py --live             # Clean duplicates
        """
    )
    
    mode_group = parser.add_mutually_exclusive_group()
    mode_group.add_argument('--dry-run', action='store_true', default=True, help='Preview changes (default)')
    mode_group.add_argument('--live', action='store_true', help='Actually clean files')
    
    args = parser.parse_args()
    dry_run = not args.live
    
    print("🧹 Clean Duplicate Category Images")
    print("="*50)
    ...
    try:
        result = clean_duplicate_images(dry_run=dry_run)
        if result:
            report_file, removed, errors = result
            print(f"\n✅ Process completed! Check report: {report_file}")
